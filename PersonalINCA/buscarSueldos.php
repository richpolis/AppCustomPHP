...
if (mysql_num_rows($result)!=0) {
	...
	$response .= "	<tr>\n".
				...
				"		<th style='width: 80px;'>Coordinaci&oacute;n / Gerencia</th>\n".
				"		<th style='width: 70px;'>Sueldo</th>\n".
				"		<th style='width: 60px;'>Porcentaje IMSS</th>\n".
	...
	while($row = mysql_fetch_array($result)) {
		...
		if ($_SESSION['nivel'] == 0) {
			$response .= "<a href='#' onclick='return modificarSueldo(".$row["id_empleado"].
						");' title='Modificar sueldo'><img src='images/page_edit.png' /></a>";					
		}
		$response .= "</td>";
					
		$response .= "		<td class='$class'>";
		if ($_SESSION['nivel'] == 0) {
			$response .= "<a href='#' onclick='return modificarSueldoAumento(".$row["id_empleado"].
						");' title='Modificar sueldo por aumento'><img src='images/page_add.png' /></a>";	
		}					
		$response .= "</td>";
		...
		$response .= "		<td class='$class'>\n".
					"		$".htmlentities($row["sueldo_bruto_mensual"])."\n".
					"		</td>\n";
		$response .= "		<td class='$class'>\n".
					"		".htmlentities($row["porcentaje_imss"])."%\n".
					"		</td>\n";
