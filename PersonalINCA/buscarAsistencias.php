...
if (mysql_num_rows($result)!=0) {
	...
	$response .= "	<tr>\n".
				...
				"		<th style='width: 200px;'>Coordinaci&oacute;n / Gerencia</th>\n";				
	if ($rb == "0") {
		$response .= "		<th style='width: 80px;'>No. de faltas</th>\n";
	}
	else {
		$response .= "		<th style='width: 80px;'>No. de archivos</th>\n";
	}	
	$response .= "		<th style='width: 20px;'></th>\n".
				"	</tr>\n";
	$c = 0;
	while($row = mysql_fetch_array($result)) {
		...
		if ($_SESSION['nivel'] == 0) {
			if ($rb == "0") {
				$response .= "<a href='#' onclick='return iniInsertar(".$row["id_empleado"].", \"".
						htmlentities($row["nombre"])." ".htmlentities($row["apellido_paterno"]).
						" ".htmlentities($row["apellido_materno"])."\");' title='Agregar Falta'><img src='images/Button-Add-icon.png' /></a>";
			}
			else {
				$response .= "<a href='#' onclick='return iniInsertarA(".$row["id_empleado"].", \"".
						htmlentities($row["nombre"])." ".htmlentities($row["apellido_paterno"]).
						" ".htmlentities($row["apellido_materno"])."\", ".date('m').", ".date('Y').");' title='Agregar Archivo Mensual de Faltas'><img src='images/Button-Add-icon.png' /></a>";
			}								
		}
		$response .= "</td>";
		$response .= "		<td class='$class'>\n".
		...
		$response .="		<td class='$class'>\n";
		if ($rb == "0") {
			$result_nofaltas = $miconexion->consulta("SELECT COUNT(id_falta) AS no FROM faltas ".
											"WHERE id_empleado = ".$row["id_empleado"].";");
			$row_nofaltas = mysql_fetch_array($result_nofaltas);
			$response .="		".htmlentities($row_nofaltas["no"])."\n";	
		}
		else {
			$result_nofaltas = $miconexion->consulta("SELECT COUNT(id_archivo_faltas) AS no FROM archivos_faltas ".
											"WHERE id_empleado = ".$row["id_empleado"].";");
			$row_nofaltas = mysql_fetch_array($result_nofaltas);
			$response .="		".htmlentities($row_nofaltas["no"])."\n";	
