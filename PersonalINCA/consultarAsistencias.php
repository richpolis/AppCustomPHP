...
if (isset($_REQUEST["modificar"])){ /*** ENVIAMOS LOS DATOS PARA MODIFICAR VIA JSON  ********/
	$query = "SELECT * FROM faltas WHERE id_empleado = $id AND id_falta = $idf";
...
else{ /*** ES PARA MOSTRAR LOS DATOS DE LAS FALTAS DEL EMPLEADO  ********/
	$query = "SELECT * FROM faltas WHERE id_empleado = $id ORDER BY fecha DESC;";
	$result = $miconexion->consulta($query);
	...
	if (mysql_num_rows($result)!=0) {
		...
		while($row = mysql_fetch_array($result)) {
			...
			if ($_SESSION['nivel'] == 0) {
				$response.= "<div style='float: left;'>".
							"<a style='float: left;' href='#' onclick='return iniModificar("
							.$row["id_empleado"].", ".$row["id_falta"].");' title='Modificar Falta'><img src='images/form_edit.png' /></a>".
							"<a style='padding-left: 10px; float: right;' href='#' onclick='return eliminarFalta(".$row["id_empleado"].", ".$row["id_falta"].
							");' title='Eliminar Falta'><img src='images/sign_remove.png' /></a></div>";
			}
			$response.= "Fecha: </div>".
						"<div class='con_valor' style='width: 70%; float: left;'>".$fecha."</div>\n";
			$response.= "	</div>\n";
		...
		$response.= "	<h2>No hay ninguna falta registrada para este(a) empleado(a).</h2>\n";
	}
