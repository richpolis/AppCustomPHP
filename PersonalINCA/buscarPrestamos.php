...
if (mysql_num_rows($result)!=0) {
	...
	$response .= "	<tr>\n".
				...
				"		<th style='width: 80px;'>No. Pr&eacute;stamos Autorizados</th>\n".
	...
	while($row = mysql_fetch_array($result)) {
		...
		if ($_SESSION['nivel'] == 0) {
			$response.=	"<a href='#' onclick='return iniInsertar(".$row["id_empleado"].", \"".
						...
						" ".htmlentities($row["apellido_materno"])."\");' title='Agregar Pr&eacute;stamo'><img src='images/Button-Add-icon.png' /></a>";					
		}
		...
		$response .="		<td class='$class'>\n";
		$result_nofaltas = $miconexion->consulta("SELECT COUNT(id_prestamo) AS no FROM prestamos ".
