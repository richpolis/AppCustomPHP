...
if (mysql_num_rows($result)!=0) {
	...
	$response .= "	<tr>\n".
				...
				"		<th style='width: 180px;'>Coordinaci&oacute;n / Gerencia</th>\n".
	...
	while($row = mysql_fetch_array($result)) {
		...
		if ($_SESSION["nivel"] == 0) {
			$response .= "<a href='#' onclick='return iniModificar(".$row["id_empleado"].
						");' title='Modificar registro de empleado'><img src='images/form_edit.png' /></a>";
		}					
		$response .= "</td>";
					
		if ($row["finiquitado"] == "1" && $_SESSION['nivel'] == 0) {
			$response .= "<td class='$class'><a href='#' onclick='return iniModificarFiniquito(".$row["id_empleado"].");' title='Modificar finiquito'><img src='images/page_edit.png' /></a></td>";
		} else if ($row["finiquitado"] == "0" && $_SESSION['nivel'] == 0){
			$response .= "<td class='$class'><a href='#' onclick='return finiquitar(".$row["id_empleado"].");' title='Finiquitar empleado'><img src='images/page_remove.png' /></a></td>";
		}
		else {
			$response .= "<td class='$class'></td>";
		}
		$response .= "<td class='$class'>";
		if ($_SESSION['nivel'] == 0) {
			$response .= "<a href='#' onclick='return eliminarEmpleado(".$row["id_empleado"].
						")' title='Eliminar registro de empleado'><img src='images/sign_remove.png' /></a>";
		}
		$response .= "		</td>\n";					
